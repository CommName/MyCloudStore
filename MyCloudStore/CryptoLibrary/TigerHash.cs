...
namespace CryptoLibrary
{
    public class TigerHash
    {
        ...
        private static readonly ulong[] S0 = {
        ...
        private static readonly ulong[] S2 = {
        ...
        private static readonly ulong[] S3 = {
        ...
        static public byte[] TigerHashAlgo(byte[] message)
        {
            ...
            {
                var originalnaDuzina = message.Length;
                var nedostaka = 512 - message.Length % 512;

                Array.Resize<byte>(ref message, originalnaDuzina + nedostaka);
                int pom = 0;
                if(originalnaDuzina % 512 < 448)
                {
                    message[originalnaDuzina] = 128;
                    pom++;
                }
                while((originalnaDuzina+pom)%512 < 448)
                {
                    message[originalnaDuzina + pom] = 0;
                    pom++;
                }
                int k = 0;
                while ((originalnaDuzina + pom + k) % 512 == 0)
                {
                    message[originalnaDuzina + pom + k] = message[k];
                    k++;
                }
            ...
            while(i < message.Length){
                UInt64[] w = new UInt64[8];

                for(var j = 0; j<8; j++)
                {
                    w[j] = (ulong)(message[i+j*8] << 56) + (ulong)(message[i + j * 8+1] << 48) + (ulong)(message[i + j * 8 + 2] << 40) + (ulong)(message[i + j * 8 + 3] << 32) + (ulong)(message[i + j * 8 + 4] << 24) + (ulong)(message[i + j * 8 + 5] << 16) + (ulong)(message[i + j * 8 + 6] << 8) + (ulong)(message[i + j * 8 + 7]);
                }
                ...
                for (uint z = 0; z <= 3; z++)
                {
                    for (var j = 0; j <= 7; j++)
                    {
                        ...
                        byte[] cBrojevi = BitConverter.GetBytes(c);

                        a -= S0[cBrojevi[0]] ^ S1[cBrojevi[2]] ^ S2[cBrojevi[4]] ^ S3[cBrojevi[6]];
                        b += S3[cBrojevi[1]] ^ S2[cBrojevi[3]] ^ S1[cBrojevi[5]] ^ S3[cBrojevi[7]];
                        b *= (z + 1);
                    }
                    if (z == 0)
                    {
            ...
            BitConverter.GetBytes(h0).CopyTo(ret, 0);
            BitConverter.GetBytes(h1).CopyTo(ret, 8);
            BitConverter.GetBytes(h2).CopyTo(ret, 16);
